...
// this algorithm will calculate a
// marginal_cost for each machine in our cluster. What does
// marginal cost mean? If each machine has a homogeneous cost that
// represents the total usage of all its resources, then the marginal cost
// is the amount by which that homogeneous cost will increase if we
// add a new task to its workload.
type Epvm struct {
	Name string
}

func (e *Epvm) SelectCandidateNodes(t task.Task, nodes []*node.Node) []*node.Node {
	return e.selectCandidateNodes(t, nodes)
}
...
func (e *Epvm) selectCandidateNodes(t task.Task, nodes []*node.Node) []*node.Node {
	...
	for node := range nodes {
		...
		if e.checkDisk(t, nodes[node].Disk-nodes[node].DiskAllocated) {
...
func (e *Epvm) checkDisk(t task.Task, diskAvailable int64) bool {
...
func (e *Epvm) Score(t task.Task, nodes []*node.Node) map[string]float64 {
	...
	for _, node := range nodes {
		cpuUsage, err := e.calculateCpuUsage(node)
		if err != nil {
			log.Printf("error calculating CPU usage for node %s, skipping: %v\n", node.Name, err)
			continue
		}
		cpuLoad := e.calculateLoad(*cpuUsage, math.Pow(2, 0.8))
		...
		newMemPercent := (e.calculateLoad(memoryAllocated+float64(t.Memory/1000), float64(node.Memory)))
...
func (e *Epvm) calculateCpuUsage(node *node.Node) (*float64, error) {
...
func (e *Epvm) calculateLoad(usage, capacity float64) float64 {
